...
export function FloatingLines({ linesGradient = ["#9a4d30", "#ff6c2d", "#ffb02e"], enabledWaves = ["top", "middle", "bottom"], lineCount = [6, 10, 14], lineDistance = [8, 6, 4], animationSpeed = .55, interactive = false, bendRadius = 5, bendStrength = -.5, mouseDamping = .05, parallax = true, parallaxStrength = .08, topWavePosition, middleWavePosition, bottomWavePosition = { x: 2, y: -.7, rotate: -1 } }: Props) {
  ...
  useEffect(() => {
    ...
    const scene = new Scene(); const camera = new OrthographicCamera(-1,1,1,-1,0,1); const geometry = new PlaneGeometry(2,2); const material = new ShaderMaterial({uniforms,vertexShader,fragmentShader,transparent:true}); scene.add(new Mesh(geometry,material)); const timer = new Timer();
    ...
    const draw=(now=0)=>{if(pageVisible&&inView&&now-lastFrame>=33){lastFrame=now;timer.update(now);uniforms.iTime.value=timer.getElapsed();if(interactive){currentMouse.lerp(targetMouse,mouseDamping);influence+=(targetInfluence-influence)*mouseDamping;uniforms.bendInfluence.value=influence;}if(parallax){currentParallax.lerp(targetParallax,mouseDamping);}renderer.render(scene,camera);}frame=requestAnimationFrame(draw);};draw();
    return()=>{cancelAnimationFrame(frame);observer.disconnect();visibilityObserver.disconnect();document.removeEventListener("visibilitychange",pageVisibility);if(interactive){renderer.domElement.removeEventListener("pointermove",move);renderer.domElement.removeEventListener("pointerleave",leave);}geometry.dispose();material.dispose();renderer.dispose();timer.dispose();container.replaceChildren();};
