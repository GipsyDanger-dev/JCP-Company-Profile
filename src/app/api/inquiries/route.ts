...
export async function POST(request: Request) {
  ...
  let stored: Response;
  try {
    stored = await fetch(`${process.env.SUPABASE_URL}/rest/v1/inquiries`, { method: "POST", headers: { apikey: process.env.SUPABASE_SERVICE_ROLE_KEY!, Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`, "Content-Type": "application/json", Prefer: "return=minimal" }, body: JSON.stringify(inquiry) });
  } catch {
    return NextResponse.json({ error: "Gagal menyimpan inquiry. Silakan coba lagi atau hubungi kami via WhatsApp." }, { status: 502 });
  }
